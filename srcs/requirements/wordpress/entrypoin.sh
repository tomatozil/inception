...
if [ "$db_name" = "" ]; then
	wp config create --dbhost=mariadb:3306 --dbname=${MYSQL_DB} --dbuser=${MYSQL_USER} --dbpass=${MYSQL_USER_PASSWORD}
fi

if [ "$wp_admin" = "" ] || [ "$wp_admin" = 'admin' ]; then
	wp core install --admin_user=${WP_ADMIN} --admin_email=${WP_ADMIN_EMAIL} --admin_password=${WP_ADMIN_PASSWORD} --url=http://jiyun.42.fr --title='inception'
fi
